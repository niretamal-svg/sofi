...
async def publish_campaign(
    ...
    try:
        ...
        if doc.get("estado") not in [EstadoCampania.BORRADOR.value, EstadoCampania.PUBLICANDO.value, EstadoCampania.PUBLICADA.value]:
            raise HTTPException(
                status_code=400,
                detail=f"Campaign must be in borrador, publicando or publicada status, current: {doc.get('estado')}",
            )

        if doc.get("estado") in [EstadoCampania.PUBLICANDO.value, EstadoCampania.PUBLICADA.value]:
            doc["id"] = str(doc.pop("_id"))
            return _campaign_data_to_response(doc)
        ...
        for portal_status in portals_status:
            try:
                ...
                if settings.enable_mocks:
                    await asyncio.sleep(1)  # Simulate short work
                    result = {
                        "success": True,
                        "url": f"https://www.{portal_doc.get('slug', 'portal')}.com/empleos/SOFI-MOCK-{campaign_id[:6]}",
                        "id_externo": f"MOCK-{portal_id[:6].upper()}",
                    }
                else:
